...
# OpenAI API 설정 (아래 ChatOpenAI에 `proxies` 인자 없음)
        ...
        if body.get("object") == "page":
            ...
            for entry in body.get("entry", []):
                ...
                for messaging in entry.get("messaging", []):
                    logger.info(f"Processing messaging: {messaging}")
                    ...
                    try:
                        ...
                        logger.debug(f"chatbot_langchain response: {response}")
                    except Exception as e:
                        logger.error(f"Error in chatbot_langchain call: {e}", exc_info=True)
                        return {"status": "error", "message": str(e)}

                    # 메시지 전송
                    try:
                        logger.debug(f"Calling send_message to {sender_id}")
