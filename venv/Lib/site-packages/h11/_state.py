...
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환It also sees the client's Request event.
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환And sometimes, server events are annotated with a _SWITCH_* event. For
 뒤에 "REDACTED_OPENAI_KEY"로 치환example, we can have a (Response, _SWITCH_CONNECT) event, which is
 뒤에 "REDACTED_OPENAI_KEY"로 치환different from a regular Response event.
#
# 3) The keep-alive machine sees the process_keep_alive_disabled() event
 뒤에 "REDACTED_OPENAI_KEY"로 치환(which is derived from Request/Response events), and this event
 뒤에 "REDACTED_OPENAI_KEY"로 치환transitions it from True -> False, or from False -> False. There's no way
 뒤에 "REDACTED_OPENAI_KEY"로 치환to transition back.
#
# 4&5) The _SWITCH_* machines transition from False->True when we get a
 뒤에 "REDACTED_OPENAI_KEY"로 치환Request that proposes the relevant type of switch (via
 뒤에 "REDACTED_OPENAI_KEY"로 치환process_client_switch_proposals), and they go from True->False when we
 뒤에 "REDACTED_OPENAI_KEY"로 치환get a Response that has no _SWITCH_* annotation.
#
...
class ConnectionState:
    ...
    def _fire_state_triggered_transitions(self) -> None:
        ...
        while True:
            ...
            #
             뒤에 "REDACTED_OPENAI_KEY"로 치환DONE -> MIGHT_SWITCH_PROTOCOL
             뒤에 "REDACTED_OPENAI_KEY"로 치환DONE -> MUST_CLOSE
            #
