...
#  OWS            = *( SP / HTAB )
 뒤에 "REDACTED_OPENAI_KEY"로 치환             ; optional whitespace
OWS = r"[ \t]*"
...
#
#   tchar          = "!" / "#" / "$" / "%" / "&" / "'" / "*"
 뒤에 "REDACTED_OPENAI_KEY"로 치환              / "+" / "-" / "." / "^" / "_" / "`" / "|" / "~"
 뒤에 "REDACTED_OPENAI_KEY"로 치환              / DIGIT / ALPHA
 뒤에 "REDACTED_OPENAI_KEY"로 치환              ; any VCHAR, except delimiters
token = r"[-!#$%&'*+.^_`|~0-9a-zA-Z]+"
...
#  obs-fold       = CRLF 1*( SP / HTAB )
 뒤에 "REDACTED_OPENAI_KEY"로 치환             ; obsolete line folding
 뒤에 "REDACTED_OPENAI_KEY"로 치환             ; see Section 3.2.4
#
# https://tools.ietf.org/html/rfc5234#appendix-B.1
#
#   VCHAR          =  %x21-7E
 뒤에 "REDACTED_OPENAI_KEY"로 치환              ; visible (printing) characters
#
...
# Actually
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환  chunk-size     = 1*HEXDIG
#
# but we impose an upper-limit to avoid ridiculosity. len(str(2**64)) == 20
chunk_size = r"({HEXDIG}){{1,20}}".format(**globals())
# Actually
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환 chunk-ext      = *( ";" chunk-ext-name [ "=" chunk-ext-val ] )
#
