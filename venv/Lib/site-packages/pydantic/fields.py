        ...
        if self.sub_fields:
            ...
            if self.model_config.smart_union and is_union(get_origin(self.type_)):
                ...
                for field in self.sub_fields:
                    ...
                    # ```
                     뒤에 "REDACTED_OPENAI_KEY"로 치환 value, error = field.validate(v, values, strict=True)
                     뒤에 "REDACTED_OPENAI_KEY"로 치환 if error is None:
                     뒤에 "REDACTED_OPENAI_KEY"로 치환     return value, None
                    # ```
                    try:
