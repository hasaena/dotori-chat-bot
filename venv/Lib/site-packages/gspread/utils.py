...
# DEPRECATION_WARNING_TEMPLATE = (
 뒤에 "REDACTED_OPENAI_KEY"로 치환 "[Deprecated][in version {v_deprecated}]: {msg_deprecated}"
# )
...
# def deprecation_warning(version: str, msg: str) -> None:
 뒤에 "REDACTED_OPENAI_KEY"로 치환 """Emit a deprecation warning.

 뒤에 "REDACTED_OPENAI_KEY"로 치환 ..note::

 뒤에 "REDACTED_OPENAI_KEY"로 치환     This warning can be silenced by setting the environment variable:
 뒤에 "REDACTED_OPENAI_KEY"로 치환     GSPREAD_SILENCE_WARNINGS=1
 뒤에 "REDACTED_OPENAI_KEY"로 치환 """

 뒤에 "REDACTED_OPENAI_KEY"로 치환 # do not emit warning if env variable is set specifically to 1
 뒤에 "REDACTED_OPENAI_KEY"로 치환 if os.getenv(SILENCE_WARNINGS_ENV_KEY, "0") == "1":
 뒤에 "REDACTED_OPENAI_KEY"로 치환     return

 뒤에 "REDACTED_OPENAI_KEY"로 치환 warnings.warn(
 뒤에 "REDACTED_OPENAI_KEY"로 치환     DEPRECATION_WARNING_TEMPLATE.format(v_deprecated=version, msg_deprecated=msg),
 뒤에 "REDACTED_OPENAI_KEY"로 치환     DeprecationWarning,
 뒤에 "REDACTED_OPENAI_KEY"로 치환     4,  # showd the 4th stack: [1]:current->[2]:deprecation_warning->[3]:<gspread method/function>->[4]:<user's code>
 뒤에 "REDACTED_OPENAI_KEY"로 치환 )
