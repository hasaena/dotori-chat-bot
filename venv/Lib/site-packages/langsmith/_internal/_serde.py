...
# IMPORTANT: This function is used from Rust code in `langsmith-pyo3` serialization,
 뒤에 "REDACTED_OPENAI_KEY"로 치환        in order to handle serializing these tricky Python types *from Rust*.
 뒤에 "REDACTED_OPENAI_KEY"로 치환        Do not cause this function to become inaccessible (e.g. by deleting
 뒤에 "REDACTED_OPENAI_KEY"로 치환        or renaming it) without also fixing the corresponding Rust code found in:
 뒤에 "REDACTED_OPENAI_KEY"로 치환           rust/crates/langsmith-pyo3/src/serialization/mod.rs
