...
def _mssql_drop_ignore(conn, ident):
    try:
        ...
        # for row in conn.exec_driver_sql(
         뒤에 "REDACTED_OPENAI_KEY"로 치환 "select session_id from sys.dm_exec_sessions "
         뒤에 "REDACTED_OPENAI_KEY"로 치환    "where database_id=db_id('%s')" % ident):
         뒤에 "REDACTED_OPENAI_KEY"로 치환log.info("killing SQL server session %s", row['session_id'])
         뒤에 "REDACTED_OPENAI_KEY"로 치환conn.exec_driver_sql("kill %s" % row['session_id'])
