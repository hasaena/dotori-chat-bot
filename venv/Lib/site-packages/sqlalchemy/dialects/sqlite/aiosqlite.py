...
class AsyncAdapt_aiosqlite_dbapi:
    ...
    def connect(self, *arg, **kw):
        ...
        # A. Because there is no way to set connection.isolation_level
         뒤에 "REDACTED_OPENAI_KEY"로 치환otherwise
        # Q. BUT HOW do you know it is SAFE ?????
        # A. The only operation that isn't safe is the isolation level set
         뒤에 "REDACTED_OPENAI_KEY"로 치환operation which aiosqlite appears to have let slip through even
         뒤에 "REDACTED_OPENAI_KEY"로 치환though pysqlite appears to do check_same_thread for this.
         뒤에 "REDACTED_OPENAI_KEY"로 치환All execute operations etc. should be safe because they all
         뒤에 "REDACTED_OPENAI_KEY"로 치환go through the single executor thread.
