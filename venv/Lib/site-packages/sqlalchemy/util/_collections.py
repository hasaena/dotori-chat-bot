...
# def has_dupes(sequence, target):
 뒤에 "REDACTED_OPENAI_KEY"로 치환if target not in sequence:
 뒤에 "REDACTED_OPENAI_KEY"로 치환    return False
 뒤에 "REDACTED_OPENAI_KEY"로 치환elif not isinstance(sequence, collections_abc.Sequence):
 뒤에 "REDACTED_OPENAI_KEY"로 치환    return False
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환idx = sequence.index(target)
 뒤에 "REDACTED_OPENAI_KEY"로 치환return target in sequence[idx + 1:]
