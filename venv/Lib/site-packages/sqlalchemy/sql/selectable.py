...
# FromClause ->
#   AliasedReturnsRows
 뒤에 "REDACTED_OPENAI_KEY"로 치환    -> Alias   only for FromClause
 뒤에 "REDACTED_OPENAI_KEY"로 치환    -> Subquery  only for SelectBase
 뒤에 "REDACTED_OPENAI_KEY"로 치환    -> CTE only for HasCTE -> SelectBase, DML
 뒤에 "REDACTED_OPENAI_KEY"로 치환    -> Lateral -> FromClause, but we accept SelectBase
 뒤에 "REDACTED_OPENAI_KEY"로 치환       w/ non-deprecated coercion
 뒤에 "REDACTED_OPENAI_KEY"로 치환    -> TableSample -> only for FromClause
