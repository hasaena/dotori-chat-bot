...
class DeferredLambdaElement(LambdaElement):
    ...
    def _resolve_with_args(self, *lambda_args):
        ...
        # expected_binds = [
         뒤에 "REDACTED_OPENAI_KEY"로 치환b._orig_key
         뒤에 "REDACTED_OPENAI_KEY"로 치환for b in self._rec.expr._generate_cache_key()[1]
         뒤에 "REDACTED_OPENAI_KEY"로 치환if b.required
        # ]
        # got_binds = [
         뒤에 "REDACTED_OPENAI_KEY"로 치환b._orig_key for b in expr._generate_cache_key()[1] if b.required
        # ]
        # if expected_binds != got_binds:
         뒤에 "REDACTED_OPENAI_KEY"로 치환raise exc.InvalidRequestError(
         뒤에 "REDACTED_OPENAI_KEY"로 치환    "Lambda callable at %s produced a different set of bound "
         뒤에 "REDACTED_OPENAI_KEY"로 치환    "parameters than its original run: %s"
         뒤에 "REDACTED_OPENAI_KEY"로 치환    % (self.fn.__code__, ", ".join(got_binds))
