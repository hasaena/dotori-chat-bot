...
class ClassManager(HasMemoized, dict):
    ...
    def _all_sqla_attributes(self, exclude=None):
        ...
        # 2. if a subclass has the same key as a superclass, include that
         뒤에 "REDACTED_OPENAI_KEY"로 치환key as part of the ordering of the superclass, because an
         뒤에 "REDACTED_OPENAI_KEY"로 치환overridden key is usually installed by the mapper which is going
         뒤에 "REDACTED_OPENAI_KEY"로 치환on a different ordering
