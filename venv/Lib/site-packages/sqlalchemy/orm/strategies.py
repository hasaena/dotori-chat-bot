...
class SubqueryLoader(PostLoader):
    ...
    def _generate_from_original_query(
        ...
        # 1. applications which expect before_compile() to be called
         뒤에 "REDACTED_OPENAI_KEY"로 치환below when we run .subquery() on this query (Keystone)
        # 2. applications which are doing subqueryload with complex
         뒤에 "REDACTED_OPENAI_KEY"로 치환from_self() queries, as query.subquery() / .statement
         뒤에 "REDACTED_OPENAI_KEY"로 치환has to do the full compile context for multiply-nested
         뒤에 "REDACTED_OPENAI_KEY"로 치환from_self() (Neutron) - see test_subqload_from_self
         뒤에 "REDACTED_OPENAI_KEY"로 치환for demo.
