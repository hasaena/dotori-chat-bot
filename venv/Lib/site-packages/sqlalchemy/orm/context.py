...
class ORMCompileState(CompileState):
    ...
    def orm_pre_session_exec(
        ...
        # 1. _result_disable_adapt_to_context=True
         뒤에 "REDACTED_OPENAI_KEY"로 치환this will disable the ResultSetMetadata._adapt_to_context()
         뒤에 "REDACTED_OPENAI_KEY"로 치환step which we don't need, as we have result processors cached
         뒤에 "REDACTED_OPENAI_KEY"로 치환against the original SELECT statement before caching.
        # 2. future_result=True.  The ORM should **never** resolve columns
         뒤에 "REDACTED_OPENAI_KEY"로 치환in a result set based on names, only on Column objects that
         뒤에 "REDACTED_OPENAI_KEY"로 치환are correctly adapted to the context.   W the legacy result
         뒤에 "REDACTED_OPENAI_KEY"로 치환it will still attempt name-based resolution and also emit a
         뒤에 "REDACTED_OPENAI_KEY"로 치환warning.
    ...
    def create_for_statement(cls, statement_container, compiler, **kw):
        ...
        if isinstance(
            ...
            # subqueryload figure out what it needs more intelligently.
             뒤에 "REDACTED_OPENAI_KEY"로 치환        for entity in self._entities:
             뒤에 "REDACTED_OPENAI_KEY"로 치환            entity.setup_compile_state(self)
...
def _legacy_determine_last_joined_entity(setup_joins, entity_zero):
    ...
    # 7. therefore can pass in just the last setup_joins record,
     뒤에 "REDACTED_OPENAI_KEY"로 치환don't need entity_zero
