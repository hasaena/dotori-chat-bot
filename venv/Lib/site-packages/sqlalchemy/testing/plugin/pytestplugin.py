...
def setup_test_methods(request):
    ...
    # 1. function level "autouse" fixtures under py3k (examples: TablesTest
     뒤에 "REDACTED_OPENAI_KEY"로 치환define tables / data, MappedTest define tables / mappers / data)
    ...
    # 4. function level fixtures defined on test functions themselves,
     뒤에 "REDACTED_OPENAI_KEY"로 치환e.g. "connection", "metadata" run next
    ...
    # 7. function level fixtures defined on test functions
     뒤에 "REDACTED_OPENAI_KEY"로 치환themselves, e.g. "connection" rolls back the transaction, "metadata"
     뒤에 "REDACTED_OPENAI_KEY"로 치환emits drop all

    # 8. pytest hook pytest_runtest_teardown hook runs, this is associated
     뒤에 "REDACTED_OPENAI_KEY"로 치환with fixtures close all sessions, provisioning.stop_test_class(),
     뒤에 "REDACTED_OPENAI_KEY"로 치환engines.testing_reaper -> ensure all connection pool connections
     뒤에 "REDACTED_OPENAI_KEY"로 치환are returned, engines created by testing_engine that aren't the
     뒤에 "REDACTED_OPENAI_KEY"로 치환config engine are disposed
    ...
    # 12. function level "autouse" fixtures under py3k (examples: TablesTest /
     뒤에 "REDACTED_OPENAI_KEY"로 치환MappedTest delete table data, possibly drop tables and clear mappers
     뒤에 "REDACTED_OPENAI_KEY"로 치환depending on the flags defined by the test class)
