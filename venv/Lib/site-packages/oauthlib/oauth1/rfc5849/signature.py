...
def signature_base_string(
    ...
    # 1.  The HTTP request method in uppercase.  For example: "HEAD",
     뒤에 "REDACTED_OPENAI_KEY"로 치환 "GET", "POST", etc.  If the request uses a custom HTTP method, it
     뒤에 "REDACTED_OPENAI_KEY"로 치환 MUST be encoded (`Section 3.6`_).
    #
    ...
    # 3.  The base string URI from `Section 3.4.1.2`_, after being encoded
     뒤에 "REDACTED_OPENAI_KEY"로 치환 (`Section 3.6`_).
    #
    ...
    # 5.  The request parameters as normalized in `Section 3.4.1.3.2`_, after
     뒤에 "REDACTED_OPENAI_KEY"로 치환 being encoded (`Section 3.6`).
    #
...
def base_string_uri(uri: str, host: str = None) -> str:
    ...
    # 2.  The host and port values MUST match the content of the HTTP
     뒤에 "REDACTED_OPENAI_KEY"로 치환 request "Host" header field.
    if host is not None:
    ...
    # 3.  The port MUST be included if it is not the default port for the
     뒤에 "REDACTED_OPENAI_KEY"로 치환 scheme, and MUST be excluded if it is the default.  Specifically,
     뒤에 "REDACTED_OPENAI_KEY"로 치환 the port MUST be excluded when making an HTTP request `RFC2616`_
     뒤에 "REDACTED_OPENAI_KEY"로 치환 to port 80 or when making an HTTPS request `RFC2818`_ to port 443.
     뒤에 "REDACTED_OPENAI_KEY"로 치환 All other non-default port numbers MUST be included.
    #
...
def collect_parameters(uri_query='', body=None, headers=None,
    ...
    # *  The query component of the HTTP request URI as defined by
     뒤에 "REDACTED_OPENAI_KEY"로 치환`RFC3986, Section 3.4`_.  The query component is parsed into a list
     뒤에 "REDACTED_OPENAI_KEY"로 치환of name/value pairs by treating it as an
     뒤에 "REDACTED_OPENAI_KEY"로 치환"application/x-www-form-urlencoded" string, separating the names
     뒤에 "REDACTED_OPENAI_KEY"로 치환and values and decoding them as defined by W3C.REC-html40-19980424
     뒤에 "REDACTED_OPENAI_KEY"로 치환`W3C-HTML-4.0`_, Section 17.13.4.
    #
    ...
    # *  The OAuth HTTP "Authorization" header field (`Section 3.5.1`_) if
     뒤에 "REDACTED_OPENAI_KEY"로 치환present.  The header's content is parsed into a list of name/value
     뒤에 "REDACTED_OPENAI_KEY"로 치환pairs excluding the "realm" parameter if present.  The parameter
     뒤에 "REDACTED_OPENAI_KEY"로 치환values are decoded as defined by `Section 3.5.1`_.
    #
    ...
    # *  The HTTP request entity-body, but only if all of the following
     뒤에 "REDACTED_OPENAI_KEY"로 치환conditions are met:
     뒤에 "REDACTED_OPENAI_KEY"로 치환 *  The entity-body is single-part.
    #
     뒤에 "REDACTED_OPENAI_KEY"로 치환 *  The entity-body follows the encoding requirements of the
     뒤에 "REDACTED_OPENAI_KEY"로 치환    "application/x-www-form-urlencoded" content-type as defined by
     뒤에 "REDACTED_OPENAI_KEY"로 치환    W3C.REC-html40-19980424 `W3C-HTML-4.0`_.

     뒤에 "REDACTED_OPENAI_KEY"로 치환 *  The HTTP request entity-header includes the "Content-Type"
     뒤에 "REDACTED_OPENAI_KEY"로 치환    header field set to "application/x-www-form-urlencoded".
    #
    # .. _`W3C-HTML-4.0`: https://www.w3.org/TR/1998/REC-html40-19980424/
...
def normalize_parameters(params) -> str:
    ...
    # 2.  The parameters are sorted by name, using ascending byte value
     뒤에 "REDACTED_OPENAI_KEY"로 치환 ordering.  If two or more parameters share the same name, they
     뒤에 "REDACTED_OPENAI_KEY"로 치환 are sorted by their value.
    key_values.sort()

    # 3.  The name of each parameter is concatenated to its corresponding
     뒤에 "REDACTED_OPENAI_KEY"로 치환 value using an "=" character (ASCII code 61) as a separator, even
     뒤에 "REDACTED_OPENAI_KEY"로 치환 if the value is empty.
    parameter_parts = ['{}={}'.format(k, v) for k, v in key_values]

    # 4.  The sorted name/value pairs are concatenated together into a
     뒤에 "REDACTED_OPENAI_KEY"로 치환 single string by using an "&" character (ASCII code 38) as
     뒤에 "REDACTED_OPENAI_KEY"로 치환 separator.
...
def _sign_hmac(hash_algorithm_name: str,
    ...
    **HMAC-SHA256**
    ...
    # 2.  An "&" character (ASCII code 38), which MUST be included
     뒤에 "REDACTED_OPENAI_KEY"로 치환 even when either secret is empty.
    key += '&'
    ...
    # digest  is used to set the value of the "oauth_signature" protocol
     뒤에 "REDACTED_OPENAI_KEY"로 치환     parameter, after the result octet string is base64-encoded
     뒤에 "REDACTED_OPENAI_KEY"로 치환     per `RFC2045, Section 6.8`.
    #
...
def _sign_rsa(hash_algorithm_name: str,
    ...
    # 1. While b2a_base64 implements base64 defined by RFC 3548. As used here,
     뒤에 "REDACTED_OPENAI_KEY"로 치환it is the same as base64 defined by RFC 2045.
    ...
    # 3. b2a_base64 produces a binary string. Use decode to produce a str.
     뒤에 "REDACTED_OPENAI_KEY"로 치환It should only contain only printable US-ASCII characters.
...
def _verify_rsa(hash_algorithm_name: str,
    ...
    try:
        ...
        # *signature base string*.
        #
         뒤에 "REDACTED_OPENAI_KEY"로 치환 RSASSA-PKCS1-V1_5-VERIFY ((n, e), M, S)
...
def sign_plaintext(client_secret, resource_owner_secret):
    ...
    # 2.  An "&" character (ASCII code 38), which MUST be included even
     뒤에 "REDACTED_OPENAI_KEY"로 치환 when either secret is empty.
    signature += '&'
