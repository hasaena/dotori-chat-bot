...
# C++ offload lib requires google-auth lib to provide the following callback:
 뒤에 "REDACTED_OPENAI_KEY"로 치환 using SignFunc = int (*)(unsigned char *sig, size_t *sig_len,
 뒤에 "REDACTED_OPENAI_KEY"로 치환         const unsigned char *tbs, size_t tbs_len)
...
def load_signer_lib(signer_lib_path):
    ...
    # func SignForPython(configFilePath *C.char, digest *byte, digestLen int,
     뒤에 "REDACTED_OPENAI_KEY"로 치환 sigHolder *byte, sigHolderLen int)
