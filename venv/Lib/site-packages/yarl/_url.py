...
class URL:
    ...
    # gen-delims  = ":" / "/" / "?" / "#" / "[" / "]" / "@"
    # sub-delims  = "!" / "$" / "&" / "'" / "(" / ")"
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / "*" / "+" / "," / ";" / "="
    # unreserved  = ALPHA / DIGIT / "-" / "." / "_" / "~"
    # URI         = scheme ":" hier-part [ "?" query ] [ "#" fragment ]
    # hier-part   = "//" authority path-abempty
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / path-absolute
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / path-rootless
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / path-empty
    ...
    # IPv6address =                            6( h16 ":" ) ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         /                       "::" 5( h16 ":" ) ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [               h16 ] "::" 4( h16 ":" ) ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *1( h16 ":" ) h16 ] "::" 3( h16 ":" ) ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *2( h16 ":" ) h16 ] "::" 2( h16 ":" ) ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *3( h16 ":" ) h16 ] "::"    h16 ":"   ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *4( h16 ":" ) h16 ] "::"              ls32
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *5( h16 ":" ) h16 ] "::"              h16
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / [ *6( h16 ":" ) h16 ] "::"
    # ls32        = ( h16 ":" h16 ) / IPv4address
     뒤에 "REDACTED_OPENAI_KEY"로 치환         ; least-significant 32 bits of address
    # h16         = 1*4HEXDIG
     뒤에 "REDACTED_OPENAI_KEY"로 치환         ; 16 bits of address represented in hexadecimal
    # IPv4address = dec-octet "." dec-octet "." dec-octet "." dec-octet
    # dec-octet   = DIGIT                 ; 0-9
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / %x31-39 DIGIT         ; 10-99
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / "1" 2DIGIT            ; 100-199
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / "2" %x30-34 DIGIT     ; 200-249
     뒤에 "REDACTED_OPENAI_KEY"로 치환         / "25" %x30-35          ; 250-255
    # reg-name    = *( unreserved / pct-encoded / sub-delims )
    # port        = *DIGIT
    # path          = path-abempty    ; begins with "/" or is empty
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-absolute   ; begins with "/" but not "//"
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-noscheme   ; begins with a non-colon segment
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-rootless   ; begins with a segment
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-empty      ; zero characters
    ...
    # segment-nz-nc = 1*( unreserved / pct-encoded / sub-delims / "@" )
     뒤에 "REDACTED_OPENAI_KEY"로 치환           ; non-zero-length segment without any colon ":"
    ...
    # relative-part = "//" authority path-abempty
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-absolute
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-noscheme
     뒤에 "REDACTED_OPENAI_KEY"로 치환           / path-empty
...
def _encode_host(host: str, validate_host: bool) -> str:
    ...
    if host and (host[-1].isdigit() or ":" in host):
        ...
        # - [2001:db8::ff00:42:8329] (contains a colon -- brackets should
         뒤에 "REDACTED_OPENAI_KEY"로 치환                         have been removed before it gets here)
