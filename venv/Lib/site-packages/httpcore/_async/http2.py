    ...
    async def _read_incoming_data(self, request: Request) -> list[h2.events.Event]:
        ...
        try:
            ...
            # 1. Save the exception and just raise it immediately on any future reads.
             뒤에 "REDACTED_OPENAI_KEY"로 치환(For example, this means that a single read timeout or disconnect will
             뒤에 "REDACTED_OPENAI_KEY"로 치환immediately close all pending streams. Without requiring multiple
             뒤에 "REDACTED_OPENAI_KEY"로 치환sequential timeouts.)
            # 2. Mark the connection as errored, so that we don't accept any other
             뒤에 "REDACTED_OPENAI_KEY"로 치환incoming requests.
        ...
        async with self._write_lock:
            ...
            try:
                ...
                # 1. Save the exception and just raise it immediately on any future write.
                 뒤에 "REDACTED_OPENAI_KEY"로 치환(For example, this means that a single write timeout or disconnect will
                 뒤에 "REDACTED_OPENAI_KEY"로 치환immediately close all pending streams. Without requiring multiple
                 뒤에 "REDACTED_OPENAI_KEY"로 치환sequential timeouts.)
                # 2. Mark the connection as errored, so that we don't accept any other
                 뒤에 "REDACTED_OPENAI_KEY"로 치환incoming requests.
