...
#include <cstdint>
 ...
 뒤에 "REDACTED_OPENAI_KEY"로 치환define G_NO_COPIES_OF_CLS(Cls) private:     \
    Cls(const Cls& other) = delete; \
    Cls& operator=(const Cls& other) = delete

 뒤에 "REDACTED_OPENAI_KEY"로 치환define G_NO_ASSIGNMENT_OF_CLS(Cls) private:  \
    Cls& operator=(const Cls& other) = delete

 뒤에 "REDACTED_OPENAI_KEY"로 치환define G_NO_COPY_CONSTRUCTOR_OF_CLS(Cls) private: \
    Cls(const Cls& other) = delete;
...
/* We used to check for GCC 4+ or 3.4+, but those compilers are
 ...
 뒤에 "REDACTED_OPENAI_KEY"로 치환define GREENLET_NOINLINE(name) __attribute__((noinline)) name
 뒤에 "REDACTED_OPENAI_KEY"로 치환define GREENLET_NOINLINE_P(rtype, name) rtype __attribute__((noinline)) name
 뒤에 "REDACTED_OPENAI_KEY"로 치환define UNUSED(x) UNUSED_ ## x __attribute__((__unused__))
#elif defined(_MSC_VER)
/* We used to check for  && (_MSC_VER >= 1300) but that's also out of date. */
 뒤에 "REDACTED_OPENAI_KEY"로 치환define GREENLET_NOINLINE(name) __declspec(noinline) name
 뒤에 "REDACTED_OPENAI_KEY"로 치환define GREENLET_NOINLINE_P(rtype, name) __declspec(noinline) rtype name
 뒤에 "REDACTED_OPENAI_KEY"로 치환define UNUSED(x) UNUSED_ ## x
#endif

#if defined(_MSC_VER)
 뒤에 "REDACTED_OPENAI_KEY"로 치환define G_NOEXCEPT_WIN32 noexcept
#else
 뒤에 "REDACTED_OPENAI_KEY"로 치환define G_NOEXCEPT_WIN32
#endif
...
// See https://github.com/python-greenlet/greenlet/pull/419
 뒤에 "REDACTED_OPENAI_KEY"로 치환 define GREENLET_BROKEN_THREAD_LOCAL_CLEANUP_JUST_LEAK 1
#else
 뒤에 "REDACTED_OPENAI_KEY"로 치환 define GREENLET_BROKEN_THREAD_LOCAL_CLEANUP_JUST_LEAK 0
#endif
