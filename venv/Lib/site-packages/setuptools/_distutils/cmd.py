...
class Command:
    ...
    # defines 'sub_commands' as a class attribute; it's a list of
     뒤에 "REDACTED_OPENAI_KEY"로 치환(command_name : string, predicate : unbound_method | string | None)
    ...
    #   initialize_options()
     뒤에 "REDACTED_OPENAI_KEY"로 치환 provide default values for all options; may be customized by
     뒤에 "REDACTED_OPENAI_KEY"로 치환 setup script, by options from config file(s), or by command-line
     뒤에 "REDACTED_OPENAI_KEY"로 치환 options
    #   finalize_options()
     뒤에 "REDACTED_OPENAI_KEY"로 치환 decide on the final values for all options; this is called
     뒤에 "REDACTED_OPENAI_KEY"로 치환 after all possible intervention from the outside world
     뒤에 "REDACTED_OPENAI_KEY"로 치환 (command-line, option file, etc.) has been processed
    #   run()
     뒤에 "REDACTED_OPENAI_KEY"로 치환 run the command: do whatever it is we're here to do,
     뒤에 "REDACTED_OPENAI_KEY"로 치환 controlled by the command's various option values
