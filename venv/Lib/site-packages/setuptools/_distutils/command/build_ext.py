...
class build_ext(Command):
    ...
    #   - Command.__init__() needs access to its share of the whole
     뒤에 "REDACTED_OPENAI_KEY"로 치환 command line (must ultimately come from
     뒤에 "REDACTED_OPENAI_KEY"로 치환 Distribution.parse_command_line())
    #   - it then calls the current command class' option-parsing
     뒤에 "REDACTED_OPENAI_KEY"로 치환 callback to deal with weird options like -D, which have to
     뒤에 "REDACTED_OPENAI_KEY"로 치환 parse the option text and churn out some custom data
     뒤에 "REDACTED_OPENAI_KEY"로 치환 structure
    #   - that data structure (in this case, a list of 2-tuples)
     뒤에 "REDACTED_OPENAI_KEY"로 치환 will then be present in the command object by the time
     뒤에 "REDACTED_OPENAI_KEY"로 치환 we get to finalize_options() (i.e. the constructor
     뒤에 "REDACTED_OPENAI_KEY"로 치환 takes care of both command-line and client options
     뒤에 "REDACTED_OPENAI_KEY"로 치환 in between initialize_options() and finalize_options())
    ...
    def run(self) -> None:  # noqa: C901
        ...
        # also allow the 'extensions' list to be a list of tuples:
         뒤에 "REDACTED_OPENAI_KEY"로 치환(ext_name, build_info)
    ...
    def build_extension(self, ext) -> None:
        ...
        #   - CFLAGS environment variable (not particularly
         뒤에 "REDACTED_OPENAI_KEY"로 치환 elegant, but people seem to expect it and I
         뒤에 "REDACTED_OPENAI_KEY"로 치환 guess it's useful)
