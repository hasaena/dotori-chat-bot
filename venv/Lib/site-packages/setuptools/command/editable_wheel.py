...
class editable_wheel(Command):
    ...
    def _run_build_subcommands(self) -> None:
        ...
        # TODO: Once plugins/customizations had the chance to catch up, replace
         뒤에 "REDACTED_OPENAI_KEY"로 치환   `self._run_build_subcommands()` with `self.run_command("build")`.
         뒤에 "REDACTED_OPENAI_KEY"로 치환   Also remove _safely_run, TestCustomBuildPy. Suggested date: Aug/2023.
        build = self.get_finalized_command("build")
...
class _TopLevelFinder:
    ...
    def template_vars(self) -> tuple[str, str, dict[str, str], dict[str, list[str]]]:
        ...
        # ^-- We need to explicitly add the legacy_namespaces to the mapping to be
         뒤에 "REDACTED_OPENAI_KEY"로 치환 able to import their modules even if another package sharing the same
         뒤에 "REDACTED_OPENAI_KEY"로 치환 namespace is installed in a conventional (non-editable) way.
...
class _EditableNamespaceFinder:  # PathEntryFinder
    ...
    def _paths(cls, fullname: str) -> list[str]:
        ...
        # 2. In the case of nested namespaces, we need to force
         뒤에 "REDACTED_OPENAI_KEY"로 치환import machinery to query _EditableNamespaceFinder again.
