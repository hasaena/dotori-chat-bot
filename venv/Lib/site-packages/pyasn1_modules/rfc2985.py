...
# Note: pkcs_9_at_smimeCapabilities is not included in the map because
 뒤에 "REDACTED_OPENAI_KEY"로 치환   the definition in RFC 5751 is preferred, which produces the same
 뒤에 "REDACTED_OPENAI_KEY"로 치환   encoding, but it allows different parameters for SMIMECapability
 뒤에 "REDACTED_OPENAI_KEY"로 치환   and AlgorithmIdentifier.
