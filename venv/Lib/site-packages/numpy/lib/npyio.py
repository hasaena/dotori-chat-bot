...
def _read(fname, *, delimiter=',', comment='#', quote='"',
    ...
    if comments is not None:
        # Note: An earlier version support two character comments (and could
         뒤에 "REDACTED_OPENAI_KEY"로 치환   have been extended to multiple characters, we assume this is
         뒤에 "REDACTED_OPENAI_KEY"로 치환   rare enough to not optimize for.
    ...
    # NOTE: ndmin works as advertised for structured dtypes, but normally
     뒤에 "REDACTED_OPENAI_KEY"로 치환   these would return a 1D result plus the structured dimension,
     뒤에 "REDACTED_OPENAI_KEY"로 치환   so ndmin=2 adds a third dimension even when no squeezing occurs.
     뒤에 "REDACTED_OPENAI_KEY"로 치환   A `squeeze=False` could be a better solution (pandas uses squeeze).
...
#
 뒤에 "REDACTED_OPENAI_KEY"로 치환        nbrows -= skip_footer
 뒤에 "REDACTED_OPENAI_KEY"로 치환        errmsg = [template % (i, nb)
 뒤에 "REDACTED_OPENAI_KEY"로 치환                  for (i, nb) in invalid if i < nbrows]
