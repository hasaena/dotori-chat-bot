...
# from packaging.tags import sys_tags
 뒤에 "REDACTED_OPENAI_KEY"로 치환 _tags = list(sys_tags())
 뒤에 "REDACTED_OPENAI_KEY"로 치환 if 'musllinux' in _tags[0].platform:
...
def assert_array_compare(comparison, x, y, err_msg='', verbose=True, header='',
    ...
    def func_assert_same_pos(x, y, func=isnan, hasval='nan'):
        ...
        # (1) all() on `masked` array scalars can return masked arrays, so we
         뒤에 "REDACTED_OPENAI_KEY"로 치환 use != True
        # (2) __eq__ on some ndarray subclasses returns Python booleans
         뒤에 "REDACTED_OPENAI_KEY"로 치환 instead of element-wise comparisons, so we cast to bool_() and
         뒤에 "REDACTED_OPENAI_KEY"로 치환 use isinstance(..., bool) checks
        # (3) subclasses with bare-bones __array_function__ implementations may
         뒤에 "REDACTED_OPENAI_KEY"로 치환 not implement np.all(), so favor using the .all() method
