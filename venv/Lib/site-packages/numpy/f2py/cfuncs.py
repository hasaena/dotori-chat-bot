...
# define NUMFROMARROBJ(typenum,ctype) \\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 if (PyArray_Check(obj)) arr = (PyArrayObject *)obj;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 else arr = (PyArrayObject *)PyArray_ContiguousFromObject(obj,typenum,0,0);\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 if (arr) {\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     if (PyArray_TYPE(arr)==NPY_OBJECT) {\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         if (!ctype ## _from_pyobj(v,(PyArray_DESCR(arr)->getitem)(PyArray_DATA(arr)),\"\"))\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         goto capi_fail;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     } else {\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         (PyArray_DESCR(arr)->cast[typenum])(PyArray_DATA(arr),1,(char*)v,1,1);\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     }\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     if ((PyObject *)arr != obj) { Py_DECREF(arr); }\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     return 1;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 }
# """
...
# define CNUMFROMARROBJ(typenum,ctype) \\
 ...
 뒤에 "REDACTED_OPENAI_KEY"로 치환         goto capi_fail;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     } else {\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         (PyArray_DESCR(arr)->cast[typenum])((void *)(PyArray_DATA(arr)),1,(void *)(v),1,1);\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     }\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     if ((PyObject *)arr != obj) { Py_DECREF(arr); }\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     return 1;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 }
...
# define CHECKDIMS(dims,rank) \\
 뒤에 "REDACTED_OPENAI_KEY"로 치환 for (int i=0;i<(rank);i++)\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     if (dims[i]<0) {\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         fprintf(stderr,\"Unspecified array argument requires a complete dimension specification.\\n\");\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환         goto capi_fail;\\
 뒤에 "REDACTED_OPENAI_KEY"로 치환     }
# """
cppmacros[
