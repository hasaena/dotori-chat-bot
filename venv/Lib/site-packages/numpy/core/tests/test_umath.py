...
class TestSpecialFloats:
    ...
    def test_sqrt_values(self, dt):
        ...
        # with np.errstate(invalid='raise'):
         뒤에 "REDACTED_OPENAI_KEY"로 치환 assert_raises(
         뒤에 "REDACTED_OPENAI_KEY"로 치환     FloatingPointError, np.sqrt, np.array(-100., dtype=dt)
    ...
    def test_unary_spurious_fpexception(self, ufunc, dtype, data, escape):
        ...
        # FIXME: skipped on MSVC:32-bit during switch to Meson, 10 cases fail
         뒤에 "REDACTED_OPENAI_KEY"로 치환    when SIMD support not present / disabled
...
def test_spacing_gfortran():
    ...
    # 32bits:
     뒤에 "REDACTED_OPENAI_KEY"로 치환   PROGRAM test_spacing
     뒤에 "REDACTED_OPENAI_KEY"로 치환    INTEGER, PARAMETER :: SGL = SELECTED_REAL_KIND(p=6, r=37)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    INTEGER, PARAMETER :: DBL = SELECTED_REAL_KIND(p=13, r=200)
    #
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(0.00001_DBL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(1.0_DBL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(1000._DBL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(10500._DBL)
    #
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(0.00001_SGL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(1.0_SGL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(1000._SGL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환    WRITE(*,*) spacing(10500._SGL)
     뒤에 "REDACTED_OPENAI_KEY"로 치환   END PROGRAM
