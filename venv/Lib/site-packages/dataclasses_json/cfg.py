...
class _GlobalConfig:
    ...
    # def json_module(self):
     뒤에 "REDACTED_OPENAI_KEY"로 치환 return self._json_module
    #
    # @json_module.setter
    # def json_module(self, value):
     뒤에 "REDACTED_OPENAI_KEY"로 치환 warnings.warn(f"Now using {value.__name__} module to handle JSON. "
     뒤에 "REDACTED_OPENAI_KEY"로 치환               f"{self._disable_msg}")
     뒤에 "REDACTED_OPENAI_KEY"로 치환 self._json_module = value
